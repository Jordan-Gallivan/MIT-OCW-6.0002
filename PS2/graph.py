...
class Digraph(object):
    """Represents a directed graph of Node and Edge objects
        self.node: set
            set of nodes in the Digraph
        self.edges: dictionary
            keys: nodes
            values: list, [edge1, edge2, ...edgen]   
    """
    def __init__(self):
        self.nodes = set([])
    ...
    def add_edge(self, edge):
        ...
        if (source in self.nodes) and (dest in self.nodes):
            # self.edges[source].append([dest, edge.get_total_distance(), edge.get_outdoor_distance()])
            self.edges[source].append(edge)
        else:
            raise ValueError('source or destination node(s) not in graph')
...
# edge1 = WeightedEdge(nodea, nodeb, 15, 10)
# edge2 = WeightedEdge(nodeb, nodec, 20, 15)
# edge3 = WeightedEdge(nodec, nodea, 25, 20)
