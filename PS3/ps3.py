...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        tiles: dict: key=(x,y), value=dirt amount (int)
        """
        
        rect_width = range(width)
        rect_height = range(height)
        self.width = width
        self.height = height
        self.tiles = {}
        
        for w in rect_width:
            for h in rect_height:
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        cur_dirt = self.tiles[(x,y)]
        
        if cur_dirt - capacity < 0:
            self.tiles[(x,y)] = 0
        else:
            self.tiles[(x,y)] = cur_dirt - capacity 
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.dirt_cleaned: positive integer; total amount of dirt cleaned 
        self.direction: float; 0<= direction < 360
        self.pos: Position Object; a valid position in room
        """
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        x = random.randint(0, self.width) + random.random()
        y = random.randint(0, self.height) + random.random()
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        if ((x,y) in self.tiles) and not self.is_position_furnished(pos):
            pos_valid = True
...
# testroom=EmptyRoom(5, 7, 2)
# print('*** Empty Room ***')
# print('Test random position:' + str(testroom.get_random_position()))
# print('Test valid position:' + str(testroom.is_position_valid(Position(4.5,6.5))))
# print('Test invalid position(x):' + str(testroom.is_position_valid(Position(-1,2))))
# print('Test invalid position(y):' + str(testroom.is_position_valid(Position(2,-2))))
# print('Test number of tiles:' + str(testroom.get_num_tiles()))
# print('Test number of clean tiles:' + str(testroom.get_num_cleaned_tiles()))

# testroom=FurnishedRoom(5, 7, 2)
# print('*** Furnished Room ***')

# testroom.add_furniture_to_room()
# testroom.add_furniture_to_room()
# furniture = testroom.get_furniture_tiles()
# singlefurnpos = furniture[random.randint(0,len(furniture) - 1)]
# randpos = testroom.get_random_position()
# print('furniture location:' + str(testroom.get_furniture_tiles()))
# print('Test random position:' + str(testroom.get_random_position()))
# print('Test valid position:' + str(testroom.is_position_valid(randpos)))
# print('Test invalid position(x):' + str(testroom.is_position_valid(Position(-1,6.5))))
# print('Test invalid position(y):' + str(testroom.is_position_valid(Position(2,-1))))
# print('Test invalid position(furniture):' + str(testroom.is_position_valid(
#     Position(singlefurnpos[0], singlefurnpos[1]))))

# rob = Robot(testroom, 2, 10)
# print('*** Robot Test ***')
# print('the robots position is: ' + str(rob.get_robot_position()))
# print('the robots direction is: ' + str(rob.get_robot_direction()))
# rob.set_robot_position(Position(0,1))
# rob.set_robot_direction(253.1)
# print('the robots position is: ' + str(rob.get_robot_position()))
# print('the robots direction is: ' + str(rob.get_robot_direction()))
# rob.set_robot_direction(540.0)
# print('test direction > 360: ' + str(rob.get_robot_direction()))
# rob.set_robot_direction(1325.0)
# print('test direction > 720: ' + str(rob.get_robot_direction()))
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        # print(str(new_pos) + str(self.room.is_position_valid(new_pos)))
        if self.room.is_position_valid(new_pos):
            self.pos = new_pos
            self.room.clean_tile_at_position(self.pos, self.capacity)
        else:
            new_direction = random.randint(0, 359) + random.random()
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    # robot_times = []    #list of individual run times
    
    
    # for i in range(num_robots):
    #     #iterate for number of robots
        
    #     room = EmptyRoom(width, height, dirt_amount)    #create a new, dirty room
    #     robot = robot_type(room, speed, capacity)       #create a new robot
    #     robot.set_robot_position(room.get_random_position())    #ensure starting position random
    #     run_time = 0        #counter for each robot (to be reinitialized for each trial)
        
    #     while (room.get_num_cleaned_tiles()/room.get_num_tiles()) < min_coverage:
    #         #iterate until percentage clean tiles > min_coverage
            
    #         robot.update_position_and_clean()   #clean tile
    #         run_time +=1                        #increment time step
            
    #     robot_times.append(run_time)    #log time for most recent robot
    #     run_time = 0                    #reset time counter
        
    # mean_time = sum(robot_times)/len(robot_times)
    ...
    for i in range(num_trials):
        ...
        for j in range(num_robots):
            ...
            robots[j].set_robot_position(room.get_random_position())    # ensure starting position random
